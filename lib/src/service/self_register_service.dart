...
class SelfRegistrationService {
  ...
  Future<SelfRegistrationParams?> getSelfRegistrationParams({RequestConfig? requestConfig}) async {
    return nullIfNotFound(
          (RequestConfig requestConfig) async {
        var response = await _tbClient.get<Map<String, dynamic>>('/api/selfRegistration/selfRegistrationParams',
            options: defaultHttpOptionsFromConfig(requestConfig));
        return response.data != null ? SelfRegistrationParams.fromJson(response.data!) : null;
      },
      requestConfig: requestConfig,
    );
  }

  Future<SelfRegistrationParams> saveSelfRegistrationParams(SelfRegistrationParams selfRegistrationParams, {RequestConfig? requestConfig}) async {
    var response = await _tbClient.post<Map<String, dynamic>>('/api/selfRegistration/selfRegistrationParams', data: jsonEncode(selfRegistrationParams),
        options: defaultHttpOptionsFromConfig(requestConfig));
  ...
  Future<String> getPrivacyPolicy({RequestConfig? requestConfig}) async {
    ...
    var response = await _tbClient.get<String>('/api/noauth/selfRegistration/privacyPolicy',
        options: options);
